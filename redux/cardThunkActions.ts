...
export const addCategoryCard = createAsyncThunk(
  'store/addCategoryCard', (card: Category) => {
...
export const addSetCard = createAsyncThunk(
  'store/addSetCard', (card: Set) => {
...
export const addFlashCard = createAsyncThunk(
  'store/addFlashCard', (card: Flashcard) => {
...
export const removeCard = createAsyncThunk(
  ...
  (payload: {id: string, type: 'category' | 'set' | 'flashcard'}) => {
    return new Promise<{id: string, type: 'category' | 'set' | 'flashcard'}>((resolve, reject) => {
...
export const updateCard = createAsyncThunk(
  ...
  (payload: {id: string, type: CardType, query: Object}) => {
    return new Promise<{id: string, type: CardType, query: Object}>(async (resolve, reject) => {
       db.update(
        { _id: payload.id },
        { $set: payload.query },
        (err: Error, numRemoved: number) => {
      ...
      //   const isFav = Object.values(payload.query).includes(true)
      // if (payload.type === 'set' && isFav) {
      //   await db.find({_id: payload.id}, (err: Error, docs: any[]) => {
      //     const updated = {id: payload.id, type: 'set' as CardType, query: docs[0]}
      //     resolve(updated)
      //   })
...
export const getCards = createAsyncThunk(
  ...
  (payload: {query: Object, type: 'category' | 'set' | 'flashcard'}) => {
    return new Promise<{type: 'category' | 'set' | 'flashcard', cards: Collection[]}>((resolve, reject) => {
...
export const getFavoriteSets = createAsyncThunk(
  'store/getFavoriteSets', () => {
    return new Promise<Set[]>((resolve, reject) => {
...
export const toggleFavoriteSet = createAsyncThunk(
  'store/toggleFavoritSet', (payload) => {
